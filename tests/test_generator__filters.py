...
from asciidoxy.generator.filters import (AllStringFilter, NoneStringFilter, IncludeStringFilter,
                                         ...
                                         FilterAction, EnumValueFilter, ExceptionFilter,
                                         filter_from_strings, InsertionFilter, combine_specs)
from asciidoxy.model import EnumValue, ThrowsClause
...
@pytest.fixture
def cpp_class_with_enum_values(cpp_class):
...
def test_insertion_filter__compound__no_filters(cpp_class_with_enum_values):
    insertion_filter = InsertionFilter()

    member_names = [member.name for member in insertion_filter.members(cpp_class_with_enum_values)]
    assert sorted(member_names) == sorted([
    ...
    enum_names = [
        enum_value.name for enum_value in insertion_filter.enum_values(cpp_class_with_enum_values)
    ]
    assert sorted(enum_names) == sorted(["kRed", "kGreen", "kBlue"])


def test_insertion_filter__compound__filter_members(cpp_class_with_enum_values):
...
def test_insertion_filter__compound__filter_enum_values(cpp_class_with_enum_values):
