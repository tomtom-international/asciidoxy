...
def test_initial_document_is_registered(context, document):
    assert context.document is document
    assert document.relative_path in context.documents
    assert context.documents[document.relative_path] is document
    assert context.document_stack == [document]


def test_create_sub_context(context, document):
    context = context
...
def test_file_with_element__different_file(context):
    context.config.multipage = True

    doc1 = context.document.with_relative_path("file_1.adoc")
    context.document = doc1
    context.insert(make_compound(language="lang", name="type1"))
    context.insert(make_compound(language="lang", name="type2"))

    doc2 = context.document.with_relative_path("file_2.adoc")
    context.document = doc2
    context.insert(make_compound(language="lang", name="type3"))
    context.insert(make_compound(language="lang", name="type4"))

    assert context.file_with_element("lang-type1") is doc1
    assert context.file_with_element("lang-type2") is doc1


def test_file_with_element__same_file(context):
    context.config.multipage = True
    ...
    assert context.file_with_element("lang-type3") is None
    assert context.file_with_element("lang-type4") is None


def test_file_with_element__not_in_singlepage(context):
    context.config.multipage = False
    ...
    assert context.file_with_element("lang-type1") is None
    assert context.file_with_element("lang-type2") is None
    assert context.file_with_element("lang-type3") is None
    assert context.file_with_element("lang-type4") is None


def test_file_with_element__element_not_found(context):
    context.config.multipage = True
    assert context.file_with_element("element-id-1") is None


def test_link_to_document__singlepage__included(context, document):
    ...
    assert context.link_to_document(linked_doc) == Path("../dir2/linked_file.adoc")


def test_link_to_document__singlepage__not_included(context, document):
    document.relative_path = Path("dir1/index.adoc")
    linked_doc = document.with_relative_path("dir2/linked_file.adoc")
    assert context.link_to_document(linked_doc) == Path("../dir2/linked_file.adoc")


def test_link_to_document__singlepage__embedded__same_file(context, document):
    document.relative_path = Path("dir1/index.adoc")
    linked_doc = document.with_relative_path("dir2/linked_file.adoc")
    document.embed(linked_doc)
    assert context.link_to_document(linked_doc) == Path("index.adoc")


def test_link_to_document__singlepage__embedded__other_file(context, document):
    document.relative_path = Path("dir1/index.adoc")
    linked_doc = document.with_relative_path("dir2/linked_file.adoc")
    other_doc = document.with_relative_path("other_file.adoc")
    other_doc.embed(linked_doc)
    assert context.link_to_document(linked_doc) == Path("../other_file.adoc")


def test_link_to_document__singlepage__embedded_multiple_times__same_file(context, document):
    document.relative_path = Path("dir1/index.adoc")
    linked_doc = document.with_relative_path("dir2/linked_file.adoc")
    document.embed(linked_doc)
...
def test_link_to_document__singlepage__embedded_multiple_times__other_files(context, document):
    document.relative_path = Path("dir1/index.adoc")
    ...
    with pytest.raises(ConsistencyError):
        context.link_to_document(linked_doc)


def test_link_to_document__multipage__included(context, document):
    document.relative_path = Path("dir1/index.adoc")
    context.config.multipage = True
    linked_doc = document.with_relative_path("linked_file.adoc")
    document.include(linked_doc)
    assert context.link_to_document(linked_doc) == Path("../linked_file.adoc")


def test_link_to_document__multipage__not_included(context, document):
    document.relative_path = Path("dir1/index.adoc")
    context.config.multipage = True
    linked_doc = document.with_relative_path("linked_file.adoc")
    assert context.link_to_document(linked_doc) == Path("../linked_file.adoc")
...
def test_link_to_document__multipage__embedded__same_file(context, document):
    document.relative_path = Path("dir1/index.adoc")
    context.config.multipage = True
    linked_doc = document.with_relative_path("dir2/linked_file.adoc")
    document.embed(linked_doc)
...
def test_link_to_document__multipage__embedded__other_file(context, document):
    document.relative_path = Path("dir1/index.adoc")
    context.config.multipage = True
    linked_doc = document.with_relative_path("dir2/linked_file.adoc")
    other_doc = document.with_relative_path("dir3/other_file.adoc")
    other_doc.embed(linked_doc)
    assert context.link_to_document(linked_doc) == Path("../dir3/other_file.adoc")


def test_link_to_document__multipage__embedded_multiple_times__same_file(context, document):
    document.relative_path = Path("dir1/index.adoc")
    context.config.multipage = True
    linked_doc = document.with_relative_path("linked_file.adoc")
    document.embed(linked_doc)
...
def test_link_to_document__multipage__embedded_multiple_times__other_files(context, document):
    document.relative_path = Path("dir1/index.adoc")
    context.config.multipage = True
    linked_doc = document.with_relative_path("linked_file.adoc")
    other_doc = document.with_relative_path("other_file.adoc")
...
def test_link_to_document__multipage__inside_embedded_file(context, document):
    context.config.multipage = True
    document.relative_path = Path("dir1/index.adoc")
    ...
    context.document_stack.insert(0, parent)
    linked_doc = document.with_relative_path("dir2/linked_file.adoc")
    document.include(linked_doc)
    assert context.link_to_document(linked_doc) == Path("dir2/linked_file.adoc")


def test_docinfo_footer_file__singlepage(context, document):
    footer_file = context.docinfo_footer_file()
...
def test_docinfo_footer_file__singlepage__included(context, document):
    parent = document.with_relative_path("parent.adoc")
    parent.is_root = True
...
def test_docinfo_footer_file__singlepage__embedded(context, document):
    parent = document.with_relative_path("parent.adoc")
    parent.is_root = True
    parent.embed(document)
    document.is_root = False
...
def test_docinfo_footer_file__multipage(context, document):
    context.config.multipage = True
...
def test_docinfo_footer_file__multipage__included(context, document):
    context.config.multipage = True
    parent = document.with_relative_path("parent.adoc")
    parent.is_root = True
    parent.include(document)
...
def test_docinfo_footer_file__multipage__embedded(context, document):
    context.config.multipage = True
    parent = document.with_relative_path("parent.adoc")
    parent.is_root = True
    parent.embed(document)
...
def test_docinfo_footer_file__multipage__embedded_multiple_times(context, document):
    context.config.multipage = True
...
def test_register_and_link_to_anchor__same_file(context, document):
    context.register_anchor("my-anchor", "anchor text")
    assert context.link_to_anchor("my-anchor") == (document.relative_path, "anchor text")


def test_register_and_link_to_anchor__different_file(context, document):
    context.document = document.with_relative_path("other.adoc")
    context.register_anchor("my-anchor", "anchor text")
    context.document = document
    assert context.link_to_anchor("my-anchor") == (Path("other.adoc"), "anchor text")


def test_register_and_link_to_anchor__no_link_text(context, document):
    context.register_anchor("my-anchor", None)
    assert context.link_to_anchor("my-anchor") == (document.relative_path, None)


def test_register_anchor__duplicate_name(context):
    context.register_anchor("my-anchor", None)
    with pytest.raises(DuplicateAnchorError):
        context.register_anchor("my-anchor", None)


def test_link_to_anchor__unknown(context):
    with pytest.raises(UnknownAnchorError):
        context.link_to_anchor("my-anchor")


def test_link_to_element__single_link(context, document):
    context.push_stack("link(\"MyElement\")", document, Package.INPUT_PACKAGE_NAME)
    context.link_to_element("my-element-id")
    context.pop_stack()

    assert "my-element-id" in context.linked
    assert context.linked["my-element-id"] == [
        [
...
def test_link_to_element__multiple_links(context, document):
    context.push_stack("link(\"MyElement\")", document, Package.INPUT_PACKAGE_NAME)
    context.link_to_element("my-element-id")
    context.pop_stack()

    other_doc = document.with_relative_path("other_file.adoc")
    context.push_stack("link(\"MyElement\")", other_doc, Package.INPUT_PACKAGE_NAME)
    context.link_to_element("my-element-id")
    context.pop_stack()
...
def test_link_to_element__nested_call_stack(context, document):
    context.push_stack("include(\"other_file.adoc\")", document, Package.INPUT_PACKAGE_NAME)
    other_doc = document.with_relative_path("other_file.adoc")
    context.push_stack("insert(\"MyElement\")", other_doc)
    context.push_stack("link(\"OtherElement\")")
    context.link_to_element("other-element-id")
    context.pop_stack()
    context.pop_stack()
    context.pop_stack()

    assert "other-element-id" in context.linked
    assert context.linked["other-element-id"] == [
        [
...
def test_insert__store_stacktrace(context, document):
    context.push_stack("include(\"other_file.adoc\")", document, Package.INPUT_PACKAGE_NAME)
    ...
    assert element.id in context.inserted
    assert context.inserted[element.id] == (document, [
        StackFrame("include(\"other_file.adoc\")", document.relative_path,
                   Package.INPUT_PACKAGE_NAME, False),
    ])


def test_insert__duplicate(context, document):
    context.config.warnings_are_errors = True

    element = make_compound(id="cpp-my_element", name="MyElement")
    other_doc = document.with_relative_path("other_file.adoc")

    context.push_stack("include(\"other_file.adoc\")", document)
    context.insert(element)
    context.pop_stack()
...
def test_output_document__singlepage__root_document(context, document):
    assert context.output_document is document


def test_output_document__multipage__root_document(context, document):
    context.config.multipage = True
    assert context.output_document is document


def test_output_document__singlepage__included_document(context, document):
    sub_doc = document.with_relative_path("sub.adoc")
    sub_context = context.sub_context(sub_doc)
    document.include(sub_doc)
    assert sub_context.output_document is document


def test_output_document__multipage__included_document(context, document):
    context.config.multipage = True
    sub_doc = document.with_relative_path("sub.adoc")
    document.include(sub_doc)
    sub_context = context.sub_context(sub_doc)
...
def test_output_document__singlepage__embedded_document(context, document):
    sub_doc = document.with_relative_path("sub.adoc")
    sub_context = context.sub_context(sub_doc)
...
def test_output_document__multipage__embedded_document(context, document):
    context.config.multipage = True
    sub_doc = document.with_relative_path("sub.adoc")
    document.embed(sub_doc)
    sub_context = context.sub_context(sub_doc)
...
def test_output_document__singlepage__multiple_embedded_document(context, document):
...
def test_output_document__multipage__multiple_embedded_document(context, document):
    context.config.multipage = True
    other_doc = document.with_relative_path("other.adoc")
    sub_doc = document.with_relative_path("sub.adoc")
    other_doc.embed(sub_doc)
