...
=== Added
  ...
  - [AD-37] Show members for other visibility levels than public. By default only public and
    protected members are shown. Use `api.filter` to change.
...
=== Changed
  ...
  - Argument `leveloffset` in `api.include` now supports `None` to prevent adding `leveloffset` in
  ...
  - [AD-32] `api.cross_document_ref` and `api.include` support a new `package_name` keyword to
  ...
  - [AD-32] For `api.cross_document_ref` the `anchor` and `link_text` arguments are now keyword
    only.
  - [AD-32] For `api.include` the `leveloffset`, `link_text`, and `link_prefix` arguments are now
    keyword only.
  ...
  - [AD-54] If no `anchor` or `link_text` is given, the title of the document is used for the link
    created by `api.cross_document_ref`. If the title cannot be read, the file name stem is used.
  - [AD-54] If no `link_text` is given, the title of the document is used for the link created by
    `api.include` in multipage mode. If the title cannot be read, the file name stem is used.
