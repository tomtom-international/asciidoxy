...
class TypeParser:
    ...
    def type_from_tokens(cls,
        ...
        if (api_reference is not None and type_ref.name and not type_ref.id
                and not cls.TRAITS.is_language_standard_type(type_ref.name)):
            api_reference.add_unresolved_reference(type_ref)
        if (api_reference is not None and type_ref.returns is not None and type_ref.returns.name
