...
def structure_and_doc_files(file_or_path: str) -> Tuple[Path, Path]:
    ...
    if input_file.stem.endswith(".structure"):
        ...
        doc_file = input_file.with_stem(f"{input_file.stem[:-10]}.doc")
        if doc_file.is_file():
        ...
        structure_file = input_file
        doc_file = input_file.with_stem(f"{input_file.stem}.doc")
        if doc_file.is_file():
            return structure_file, doc_file

        structure_file = input_file.with_stem(f"{input_file.stem}.structure")
        doc_file = input_file
