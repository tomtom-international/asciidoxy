...
class DocParser:
    ...
    def parse_structure(self, data, namespace: Optional[str] = None):
        ...
        self.documentation[full_name] = data.get(Key.Comment)
...
class StructureParser:
    ...
    def parse_as_compound(self, data, namespace: Optional[str] = None) -> Compound:
        ...
        compound.brief, compound.description = parse_comment(self.documentation.get(compound.full_name, None))

        if compound.kind == "struct":
            compound.members = [self.parse_as_compound(sub_data, compound.full_name)
                                for sub_data in data.get(Key.Substructure, [])]
        elif compound.kind == "function":
            compound.params = [self.parse_as_parameter(sub_data, namespace)
                               for sub_data in data.get(Key.Substructure, [])
    ...
    def parse_as_parameter(self, data, namespace: Optional[str] = None) -> Parameter:
        ...
        if type_name:
            param.type = TypeRef(name=type_name,
                                 language=self.TAG,
                                 namespace = namespace)
